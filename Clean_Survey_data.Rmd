...
#Plot Boxplot
ggplot(df, aes(y=dminutes(timestamp_5))) + geom_boxplot() +cd_theme()  + scale_colour_manual("red", aesthetics = c("colour", "fill")) + 
...
ggplot(df, aes(x=id, y= dminutes(timestamp_5))) + geom_point() +cd_theme() 
```
...
df$tag_time_over_45 <- ifelse(df$timestamp_5 >= dminutes(45), 1, 0 )
df$tag_time_over_60 <- ifelse(df$timestamp_5 >= dminutes(60), 1, 0 )
...
df %>%
  ...
  ggplot(aes(y=timestamp_5, x= as.factor(age_group) ,fill= as.factor(media_filter))) + geom_boxplot()  +
  cd_theme() + scale_fill_brewer( palette = "Dark2")
